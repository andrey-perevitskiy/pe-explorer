...
main (int argc, char * argv [])
{
	...
	if (argc != 2) {
		...
		do {
			...
			if (strstr(fname, ".exe") == NULL
                    && strstr(fname, ".dll") == NULL) {
				MessageBox(winh,
	...
	spl_hopt = spl_alloc("IMAGE_OPTIONAL_HEADER32", hopt_info_get(hopt));
	...
	for (i = 0; i < hfile->NumberOfSections; i++) {
		...
		if (strcmp((const char *) (hsec + i)->Name, ".edata") == 0) {
			struct pe_info pinfo = {
                bytes, hfile, hopt, hsec
            };
