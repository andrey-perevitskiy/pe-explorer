...
{
	buf_printf(spl->x, spl->y, FOREGROUND_WHITE | 0, spl->offset + 3, "%s ...",
        spl->caption);
...
spl_move (struct spoiler * spl, int y)
{
	...
	buf_printf(spl->x, spl->y, FOREGROUND_WHITE | 0, size, "%s", buf);
...
spl_data_show (const struct spoiler * spl)
{
	...
	for (i = 0; i < spl->items_q; i++)
		buf_printf(spl->x + 4, spl->y + i + 1, FOREGROUND_WHITE | 0,
			strlen(spl->data[i]), "%s", spl->data[i]);
}
...
spl_data_hide (const struct spoiler * spl)
{
	...
	for (i = 0; i < spl->items_q; i++) {
		...
		buf_printf(spl->x + 4, spl->y + i + 1, FOREGROUND_WHITE | 0, size,
            "%s", buf);
